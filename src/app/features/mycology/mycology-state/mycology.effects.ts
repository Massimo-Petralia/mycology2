      ...
      .pipe(
        switchMap((iconographicContainer) => {
          ...
          if (!requestPayload.mushroom.id) {
            ...
            actions = [...actions,  MycologyActions.createMushroomRequest({
...
saveMycologyData$ = createEffect(() =>//creteMycology
this.actions$.pipe(
  ...
  mergeMap((requestPayload) => {
    ...
    if ( // NON HA SENSO QUESTA CONDIZIONE
      !requestPayload.mushroom.id &&
      requestPayload.iconographicContainer.formiconographyarray.length !== 0
    ) {
      return of(
    ...
    if ( // NON SAI SE QUESTO FUNGO ESISTE O NON ESISTE
